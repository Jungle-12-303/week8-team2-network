...
static void sql_build_near_excerpt(const char *cursor, char *buffer,
    size_t buffer_size)
{
    size_t length = 0;
...
static void sql_set_syntax_error(SQLResult *result, const char *cursor)
{
    ...
    snprintf(result->error_message, sizeof(result->error_message),
        ...
        "check the manual that corresponds to your sql processor2 "
        "version for the right syntax to use near '%s' at line 1",
        near_excerpt);
}

static void sql_set_unknown_column_error(SQLResult *result,
    const char *column, const char *clause_name)
{
...
static int sql_match_keyword(const char **cursor, const char *keyword)
{
    ...
    while (*word != '\0') {
        if (tolower((unsigned char)*start) !=
...
static int sql_parse_identifier(const char **cursor, char *buffer,
    size_t buffer_size)
{
    size_t length = 0;
...
static int sql_parse_string(const char **cursor, char *buffer,
    size_t buffer_size)
{
    size_t length = 0;
...
static int sql_parse_comparison(const char **cursor,
...
static SQLResult sql_execute_insert(Table *table, const char *input)
{
    ...
    result = sql_make_result(SQL_STATUS_SYNTAX_ERROR, SQL_ACTION_NONE);
    cursor = input;
...
static SQLResult sql_execute_select(Table *table, const char *input)
{
    ...
    if (!sql_match_char(&cursor, '*')) {
        if (!sql_parse_identifier(&cursor, selected_column,
                sizeof(selected_column))) {
            sql_set_syntax_error(&result, cursor);
            return result;
        }

        if (!sql_is_known_column(selected_column)) {
            sql_set_unknown_column_error(&result, selected_column,
                "field list");
            return result;
        }
    ...
    if (strcasecmp(column, "id") == 0) {
        if (!sql_parse_int(&cursor, &int_value) ||
            !sql_match_statement_end(&cursor)) {
            sql_set_syntax_error(&result, cursor);
            return result;
        }

        if (!table_find_by_id_condition(table, comparison, int_value,
                &result.records, &result.row_count)) {
            result.status = SQL_STATUS_ERROR;
            return result;
        }
        ...
        if (!sql_parse_string(&cursor, text_value, sizeof(text_value)) ||
            !sql_match_statement_end(&cursor)) {
            sql_set_syntax_error(&result, cursor);
            return result;
        }

        if (!table_find_by_name_matches(table, text_value,
                &result.records, &result.row_count)) {
            result.status = SQL_STATUS_ERROR;
            return result;
        }
        ...
        if (!table_find_by_age_condition(table, comparison, int_value,
                &result.records, &result.row_count)) {
            result.status = SQL_STATUS_ERROR;
            return result;
        }
    ...
    result.record = (result.row_count > 0) ? result.records[0] : NULL;
    result.status = (result.row_count == 0) ?
        SQL_STATUS_NOT_FOUND : SQL_STATUS_OK;
    result.action = SQL_ACTION_SELECT_ROWS;
    return result;
}
...
SQLResult sql_execute(Table *table, const char *input)
{
    ...
    if (sql_match_keyword(&cursor, "EXIT") ||
        sql_match_keyword(&cursor, "QUIT")) {
        if (sql_match_statement_end(&cursor)) {
