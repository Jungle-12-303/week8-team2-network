...
발표용 데모는 손으로 한 줄씩 입력하기보다, 배치파일 하나로 순서대로 돌리는 편이 훨씬 보기 좋다.

권장 실행 래퍼:

- [`scripts/demo/demo_scenario.bat`](../../../scripts/demo/demo_scenario.bat)

이 배치파일은 기존 셸 스크립트를 순서대로 감싸서 실행한다.
...
Windows에서 실행할 때는 저장소 루트에서 아래 명령을 사용한다.

```bat
scripts\demo\demo_scenario.bat
```

포트 번호를 바꾸고 싶으면 인자를 넘길 수 있다.

```bat
scripts\demo\demo_scenario.bat 18080
```

실제 내부 로직은 셸 스크립트에서 수행되므로, `sh.exe`가 PATH에 있어야 한다.
