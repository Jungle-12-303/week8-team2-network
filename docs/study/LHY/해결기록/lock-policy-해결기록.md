...
문제는 DB 접근 단계에서 읽기와 쓰기의 공정성 정책이 명시적으로 정해져 있지 않다는 점이다.
...
현재 `sql_processor/table.c`에서는 테이블을 버킷 단위로 나누고, 각 버킷마다 공정 정책을 가진 커스텀 rwlock을 둔다.
그리고 `server/server.c`와 `server/api.c`는 이 테이블에 대해 별도 공정성 정책을 직접 구현하지 않고, 버킷 락에 위임한다.
...
즉, 현재 코드는 락이 공정성을 직접 관리한다.
writer가 기다리면 새로운 reader 진입을 막아, 한쪽이 계속 끼어드는 상황을 줄인다.
...
### 3-3. 공정성은 락 정책에 의해 관리되기 때문이다

현재 코드에는 writer가 대기 중일 때 새로운 reader를 막는 정책이 들어 있다.
즉, **read-heavy 상황에서 writer가 계속 밀리는 문제를 줄이도록 설계했다.**

이건 잘못된 구현이 아니라, **공정성을 의식한 정책이 들어간 상태**에 가깝다.
...
우리 프로젝트는 원래 **명시적으로 읽기 우선 / 쓰기 우선 / 공정 락 중 하나를 선택한 상태가 아니다.**

이 브랜치에서는 버킷 락을 공정 정책을 가진 커스텀 rwlock으로 바꾸고, writer starvation을 줄이는 방향으로 정리했다.

현재 상태를 정확히 말하면:

- 버킷 단위로 `FairRWLock`을 사용한다.
- writer가 기다리는 동안 새로운 reader 진입을 막아 starvation을 줄인다.
- strict FIFO는 아니지만, 읽기/쓰기 공정성을 위해 reader barging을 제한한다.
- 테이블 전체가 아니라 버킷별로 락을 분리해서 경쟁 범위를 줄인다.

즉, 현재 프로젝트는 "단순 rwlock"이 아니라 "공정성을 고려한 rwlock"을 사용한다.
...
현재 프로젝트의 락 정책은 **버킷 단위 `FairRWLock`을 쓰는 공정성 고려 구조**다.
그래서 read-heavy 상황에서도 writer starvation을 줄이도록 설계했다.
...
이 중에서 우리 프로젝트처럼 `SELECT *`와 `INSERT`가 섞여 있고, 동시성 테스트에서 writer가 밀리는 현상을 줄이고 싶다면 **공정락**이 가장 균형 잡힌 선택에 가깝다.

한 줄로 요약하면:

> 지금 프로젝트는 버킷 단위로 “공정한 락”을 적용해 둔 상태이고, 다음 개선 후보는 strict FIFO나 MVCC 같은 더 강한 공정/동시성 전략이다.
