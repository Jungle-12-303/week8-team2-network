...
이 문서 묶음은 정답집이 아닙니다. 목적은 `tiny`와 `proxy`를 대신 구현해 주는 것이 아니라, 구현 전에 이해해야 할 개념과 흐름을 잡아 주는 것입니다.

이 패키지는 다음 순서로 읽는 것을 권장합니다.
...
이 학습의 최종 목표는 아래 5가지를 하나의 흐름으로 이해하는 것입니다.
